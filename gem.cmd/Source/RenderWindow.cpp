...
bool RenderWindow::_bFirstInstance = true;
TTF_Font* RenderWindow::mainFont = nullptr;

using namespace std;

RenderWindow::RenderWindow(const char* title, int buff_width, int buff_height, IAudioQueue* queue, bool vsync, float custom_scale /*= 1*/) :
...
float RenderWindow::GetDpiAwareScalingFactor()
{
	if (_bFirstInstance)
	{
...
void RenderWindow::DrawFrame(const ColourBuffer& new_frame)
{
	void* ptr;
	int pitch;
...
void RenderWindow::DrawRect(int w, int h, int x, int y, const GemColour& colour)
{
...
void RenderWindow::DrawString(const char* cstr, const GemColour& colour, int size, int x, int y)
{
	LoadFonts();
	if (mainFont == nullptr)
		return;
...
void RenderWindow::DrawString(const char* cstr, const SDL_Color& sdl_colour, TTF_Font* font, int size, int x, int y)
{
...
void RenderWindow::DrawDebugText(string str)
{
	LoadFonts();
	if (mainFont == nullptr)
		return;
...
void RenderWindow::Fill(const GemColour& colour)
{
	SDL_SetRenderDrawColor(renderer, colour.Red, colour.Green, colour.Blue, colour.Alpha);
	SDL_RenderClear(renderer);
}

void RenderWindow::ZeroBuffer()
{
...
void RenderWindow::MonitorFrameRate()
{
...
const void* RenderWindow::Handle()
{
	if (!nativeHandle)
	{
		SDL_SysWMinfo info;
		...
		if (SDL_GetWindowWMInfo(window, &info))
			nativeHandle = (void*)info.info.win.window;
	}
