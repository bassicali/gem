...
class RenderWindow : public IDrawTarget
{
	public:
		RenderWindow(const char* title, int buff_width, int buff_height, IAudioQueue* queue, bool vsync, float scale = 1);
		~RenderWindow();
		...
		const SDL_Window* SDLWindowPtr() const { return window; }
